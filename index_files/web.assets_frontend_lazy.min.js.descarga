...
$(document).ready(function(){var today=new Date();var hour=today.getHours();if(hour>=17){$('.js_check_availability').hide();$('.js_timer').show();var openingTime=new Date();openingTime.setHours(9);openingTime.setMinutes(0);openingTime.setSeconds(0);var remainingTime=openingTime.getTime()-today.getTime();var seconds=Math.floor((remainingTime/1000)%60);var minutes=Math.floor((remainingTime/1000/60)%60);var hours=Math.floor((remainingTime/1000/60/60)%24);$('.js_timer').text('Lo siento, estamos cerrados. Abrimos en '+hours+'h '+minutes+'m '+seconds+'s.');}else{$('.js_check_availability').show();$('.js_timer').hide();}});;
